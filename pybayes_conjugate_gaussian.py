...
def gaussian_stats(data, mu0, n0, nu0, lam0, prob):
    ...
    入力
        data: データ
        mu0: 平均の条件付き事前分布(正規分布)の平均
        n0: 平均の条件付き事前分布(正規分布)の精度パラメータ
        nu0: 分散の事前分布(逆ガンマ分布)の形状パラメータ
        lam0: 分散の事前分布(逆ガンマ分布)の尺度パラメータ
        prob: 区間確率(0 < prob < 1)
    出力
        results: 事後統計量のデータフレーム
        mu_star: 平均条件月事後分布(正規分布)の平均
        n_star: 平均条件月事後分布(正規分布)の精度パラメータ
    ...
    stats_string = ['平均', '中央値', '最頻値', '標準偏差', '信用区間(下限)', '信用区間(上限)',
                    'HPD区間(下限)', 'HPD区間(上限)']
    params_string = ['平均 $\\mu$',  '分散 $\\sigma^2$']
    results = pd.DataFrame(stats, index=params_string, columns=stats_string)
