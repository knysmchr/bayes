...
def regression_stats(y, X, b0, A0, nu0, lam0, prob):
    ...
    入力
        y: 被説明変数
        X: 説明変数
        b0: 回帰係数の条件付き事前分布(多変量正規分布)の平均
        A0: 回帰係数の条件付き事前分布(多変量正規分布)の精度行列
        nu0: 誤差項の分散の事前分布(逆ガンマ分布)の形状パラメータ
        lam0: 誤差項の分散の事前分布(逆ガンマ分布)の尺度パラメータ
        prob: 区間確率(0 < prob < 1)
    出力
        results: 事後統計量のデータフレーム
        b_star: 回帰係数の条件付き事後分布(多変量正規分布)の平均
        A_star: 回帰係数の条件付き事後分布(多変量正規分布)の精度行列
    ...
    nu_star = y.size + nu0
    lam_star = np.square(y - X.dot(b_ols)).sum() + (b0 - b_ols).T.dot(C_star).dot(b0 - b_ols) + lam0
    ...
    stats_string = ['平均', '中央値', '最頻値', '標準偏差', '信用区間(下限)', '信用区間(上限)', 'HPD区間(下限)', 'HPD区間(上限)']
